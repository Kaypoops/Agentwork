...
function sha256(data) {
  return new DataHasher(HashAlgorithm.SHA256).update(data).digest().then((h) => h.data);
}
async function deriveDirectAddress(publicKey) {
  const tokenTypeCbor = CborSerializer.encodeByteString(hexToBytes(UNICITY_TOKEN_TYPE_HEX));
...
var SphereTokenEngine = class {
  ...
  async submitAndAwaitProof(certificationData, transaction, failLabel, failCode, options) {
    ...
    try {
      ...
      if (err instanceof Error && err.message === `Invalid inclusion proof status: ${InclusionProofVerificationStatus.TRANSACTION_HASH_MISMATCH}`) {
...
var SelfIssuedUnicityIdMinter = class {
  ...
  async mintUnicityIdToken(name, options) {
    ...
    try {
      ...
      throw new SphereError(
        `Unicity ID mint failed for "@${name}": ${detail}` + (response.status !== CertificationStatus.SUCCESS ? ` (certification status: ${response.status})` : ""),
        "AGGREGATOR_ERROR"
      );
    }
  }
